        ...
        btnConsultar.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response && response.ok && result && result.success && result.data) {
                    ...
                    let msg = 'CNPJ não encontrado.';
                    if (status === 502 || status === 504) msg = 'Serviço de CNPJ temporariamente indisponível. Tente novamente.';
                    else if (result && result.error) msg = result.error;
                    console.warn('Falha na consulta de CNPJ:', { status, result });
                    mostrarFeedbackCNPJ(msg, 'text-red-600');
                }
            } catch (error) {
                console.error('Erro na consulta de CNPJ:', error);
                const msg = error.message.includes('tempo limite')
                    ? 'Serviço de CNPJ demorou muito para responder. Tente novamente.'
                    : 'Serviço de CNPJ indisponível. Preencha os dados manualmente.';
                mostrarFeedbackCNPJ(msg, 'text-red-600');
            } finally {
